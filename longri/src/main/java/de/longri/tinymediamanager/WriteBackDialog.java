...
public class WriteBackDialog extends TmmDialog implements ActionListener {
  private static final long           serialVersionUID = -5054005564554148578L;
  /**
   * @wbp.nls.resourceBundle messages
   */
  private static final ResourceBundle BUNDLE           = ResourceBundle.getBundle("messages", new UTF8Control()); //$NON-NLS-1$
  private static final Logger         log              = LoggerFactory.getLogger(WriteBackDialog.class);
  ...
  private final WriteNfoBack writeNfoBack;
  private       JButton      btnClose;

  public WriteBackDialog(FileHandle source, FileHandle target) {
    super("Write NFO back to Server", "log"); //$NON-NLS-1$
    this.writeNfoBack = new WriteNfoBack(source, target);
      ...
      if (isStarted) {
        ...
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
          @Override protected Void doInBackground() throws Exception {
            ...
            writeNfoBack.extract();
            long elapsed = System.currentTimeMillis() - start;
            log.info(" ");
            log.info("-----------------------------------------------------------------------------");
            log.info("Write back ready after {} sec", elapsed / 1000);
            log.info("-----------------------------------------------------------------------------");
